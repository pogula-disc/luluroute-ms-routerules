...
@Slf4j
@Component
public class TForceECOMMRuleHelper {
	...
	public void updatePlannedDeliveryDate(RouteRules shipOptions, ShipmentInfo shipmentInfo,
		...
		shipOptions.getRulesInclude().forEach(shipOption -> {
			try {
				...
				shipOption.setPlannedDeliveryDate(
						getPlannedDeliveryDateForEcomm(shipOption, shipmentInfo, entityProfile));
	...
	public long getPlannedDeliveryDateForEcomm(RulesInclude shipOption, ShipmentInfo shipmentInfo,
			EntityPayload entityProfile) throws DefaultTransitTimeFailureException {
		...
		try {
			...
			return PlannedDeliveryDateRule.calculatePlannedDateOfDelivery(shipOption.getPlannedShipDate(),
					entityProfile.getTimezone(), "0", "0", dayMask, dayMask, dayMask, defaultTransitdays,
					defaultTransitdays, holidays);
		} catch (Exception e) {
